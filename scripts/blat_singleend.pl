...
if ( $GFSERVER ) {
    ...
    &mcall("MARK READS ALIGNMENT STATUS FROM GFCLIENT OUTPUT",\&mark_reads_status_for_singleend, $psl_fn, $fasta_fn);
} else {
    ...
    &mcall(
          ...
          &mcall("MARK READS ALIGNMENT STATUS",
           \&mark_reads_status_for_singleend, $psl_fn, $fasta_fn);
}
...
{
    print <<"END_USAGE";
Usage: $0 --fasta=read.fa --genome_dir=/data/genome --genome_2bit_filename=hg19.2bit --processes 2 

--help                  Shows this help message
--fasta=name          Name of read file, fasta file only
--genome=dir            Directory of genome sequences 
